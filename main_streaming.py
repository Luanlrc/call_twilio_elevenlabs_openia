...
async def stream_audio_to_twilio(texto, websocket, stream_sid, voz_escolhida=ELEVENLABS_VOICE_ID):
    ...
    try:
        ...
        # Usa streaming real da versão 2.7.1 - API correta baseada na documentação
        audio_stream = elevenlabs.text_to_speech.stream(
            ...
            optimize_streaming_latency=3,  # Ajusta para melhor qualidade
            output_format="mp3_44100_128"  # Melhor qualidade de áudio
        )
        
        # Buffer para acumular chunks menores
        buffer_chunks = []
        chunk_count = 0
        ...
        for chunk in audio_stream:
            if isinstance(chunk, bytes):
                ...
                # Envia chunks em grupos maiores para reduzir cortes
                if chunk_count >= 3 or len(b''.join(buffer_chunks)) >= 8192:
                    # Combina chunks em um buffer maior
                    combined_chunk = b''.join(buffer_chunks)
                    
                    # Converte chunk para G711 μ-law base64
                    ...
                    # Delay ajustado para melhor sincronização
                    #await asyncio.sleep(0.5)  # Aumentado para 50ms
